...
class Context {
  ...
  createOscillator() {
    const oscillator = Object.assign(new AudioNode(), {
...
test('첫 입력 전에는 무음, 절취 진행만 속도 연동·간격 제한, 종료 시 감쇠', async () => {
  ...
  try {
    ...
    sound.swipe(); sound.swipe(); sound.confirm('C');
    assert.deepEqual(context.sources.slice(-3).map((source) => source.buffer.clip), [2, 2, 3]);
    assert.equal(context.oscillators.length, 0, '일반 등급에는 선행 암시음이나 선율을 더하지 않음');
  } finally {
...
test('지원하지 않는 오디오 환경이나 음원 로딩 실패도 앱 흐름을 막지 않는다', async () => {
  ...
  try {
    ...
    unsupported.swipe(); unsupported.reveal('UR'); unsupported.confirm('UR');
    ...
    missing.cut(0.1, 2); missing.swipe(); missing.reveal('C'); missing.confirm('C');
...
test('카드 공개·확정은 C/R/SR/UR 등급에 맞는 짧은 음형을 재생한다', async () => {
  const originalContext = globalThis.AudioContext;
  const originalFetch = globalThis.fetch;
  let context;
  globalThis.AudioContext = class extends Context { constructor() { super(); context = this; } };
  ...
  try {
    const sound = new SoundEffects();
    await sound.unlock();
    for (const [rarity, noteCount] of [['C', 0], ['R', 1], ['SR', 2], ['UR', 3]]) {
      const before = context.oscillators.length;
      sound.reveal(rarity);
      assert.equal(context.sources.at(-1).buffer.clip, 2, `${rarity} 공개의 공통 카드 소리`);
      assert.equal(context.oscillators.length - before, noteCount, `${rarity} 공개 음형`);
      assert.ok(context.oscillators.slice(before).every(({ starts }) => starts[0] > context.currentTime));
      if (rarity === 'SR') assert.ok(context.oscillators.slice(before).every(({ type }) => type === 'triangle'));

      const beforeConfirm = context.oscillators.length;
      sound.confirm(rarity);
      assert.equal(context.sources.at(-1).buffer.clip, 3, `${rarity} 확정의 공통 효과음`);
      assert.equal(context.oscillators.length - beforeConfirm, noteCount, `${rarity} 확정 음형`);
    }
  } finally {
    globalThis.AudioContext = originalContext;
    globalThis.fetch = originalFetch;
